...
static ConfigurationEvaluation EvaluateConfiguration(string samplesDirectory, int refinedBoundingArea, int polygonBoundingArea)
{
    ...
    processStartInfo.Environment["AUTOMATON_MIN_REFINED_COMPONENT_BOUNDING_AREA"] = refinedBoundingArea.ToString(CultureInfo.InvariantCulture);
    processStartInfo.Environment["AUTOMATON_MIN_POLYGON_BOUNDING_AREA"] = polygonBoundingArea.ToString(CultureInfo.InvariantCulture);
...
static ProcessStartInfo BuildChildProcessStartInfo(string samplesDirectory)
{
    ...
    foreach (System.Collections.DictionaryEntry environmentVariable in Environment.GetEnvironmentVariables())
    {
        ...
        if (string.IsNullOrWhiteSpace(variableName) ||
            ...
            !variableName.StartsWith("AUTOMATON_", StringComparison.OrdinalIgnoreCase))
        {
            continue;
        }
...
{
    Console.WriteLine("Automaton.Tuning");
    Console.WriteLine("Scores generated sample annotations against the expected gold-standard overlays.");
    Console.WriteLine();
    Console.WriteLine("Usage:");
    Console.WriteLine("  dotnet run --project Automaton.Tuning -- [--samples <path>] [--refined <area>] [--polygon <area>]");
    Console.WriteLine("  dotnet run --project Automaton.Tuning -- --sweep [--samples <path>] --refined-values 10000,15000 --polygon-values 30000,35000");
    Console.WriteLine();
    Console.WriteLine("Options:");
    Console.WriteLine("  --samples         Samples directory. Defaults to Automaton/bin/Debug/net9.0-windows/samples");
...
static ConfigurationEvaluation EvaluateCurrentProcessConfiguration(string samplesDirectory)
{
    ...
    var refinedBoundingArea = ReadAreaFromEnvironment("AUTOMATON_MIN_REFINED_COMPONENT_BOUNDING_AREA", 15_000);
    var polygonBoundingArea = ReadAreaFromEnvironment("AUTOMATON_MIN_POLYGON_BOUNDING_AREA", 35_000);
    ...
    bool ShowHelp)
{
    ...
    private static string GetDefaultSamplesDirectory()
    {
        ...
        var discoveryOutputDirectory = Path.GetFullPath(Path.Combine(startupDirectory, "..", "..", "..", "..", "Automaton", "bin", "Debug", "net9.0-windows", "samples"));
